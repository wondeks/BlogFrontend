...
import TagButton from "@/components/Blogs/TagButton";
...
const BlogsDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchBlogDetails = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:5000/api/postss/${id}`);
        setBlog(response.data);
      } catch (error) {
        console.error("Failed to fetch blogs details:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!blog) return <div className="pt-32 text-center text-red-500">Blogs is not found!</div>;
